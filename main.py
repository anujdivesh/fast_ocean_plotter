...
async def generate_plot_ocean(
    ...
    try:
        ...
        if use_cache:
            ...
            buf.seek(0)  # Reset buffer position after writing to file
...
async def generate_plot(
    ...
    plt.plot(x, y)
    plt.title(f"Sine Wave (A={amplitude}, F={frequency})")
    ...
    if "application/json" in accept:
        return JSONResponse({
            "amplitude": amplitude,
            "frequency": frequency,
            "data": y.tolist()  # Convert numpy array to list
        })
    else:
